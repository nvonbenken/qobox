...
import { convertDurationToSeconds } from "../../utils/parseTime";
...
const defaultPresence = {
	largeImageKey: "qobox-icon",
	largeImageText: `Qobox ${app.getVersion()}`,
	instance: false,
...
const updateActivity = () => {
	const showIdle =
		settingsStore.get<string, boolean>(settings.discord.showIdle) ?? true;
...
const getActivity = (): SetActivity => {
	...
	if (mediaInfo.status === MediaStatus.paused) {
		presence.details =
			settingsStore.get<string, string>(settings.discord.idleText) ??
			"Browsing Qobuz";
	} else {
		const showSong =
			settingsStore.get<string, boolean>(settings.discord.showSong) ?? false;
		if (showSong) {
			...
			presence.details =
				settingsStore.get<string, string>(settings.discord.usingText) ??
				"Playing media on Qobuz";
		}
	}
	...
	function getFromStore() {
		const includeTimestamps =
			settingsStore.get<string, boolean>(settings.discord.includeTimestamps) ??
			true;
		const detailsPrefix =
			settingsStore.get<string, string>(settings.discord.detailsPrefix) ??
			"Listening to ";
		const buttonText =
			settingsStore.get<string, string>(settings.discord.buttonText) ??
			"Play on Qobuz";
...
const connectWithRetry = async (retryCount = 0) => {
	try {
		...
		for (const event in Object.values(globalEvents)) {
